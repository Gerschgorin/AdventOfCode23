...
def update(list_diff):  # input is a list of lists, with the last one zero. UPDATE BUT ADD AT THE END
    ...
    for i in range(len(list_diff)-2, -1, -1): # start from the last non constant list to the first
        list_diff[i].append(list_diff[i][0]-list_diff[i+1][-1])  
#the last term is the first of the current - last of the previous list
